...
- **Лобби на WebGL подтверждено рабочим** (v1.7.6): полная цепочка `onopen → сокет открыт → join →
  0x81 SJoined → auth (свежий токен) → ev:lobby → лобби создано` проходит на платформе.
- **Убран `.ConfigureAwait(false)` из `IVhrProfile` и `IVhrAchievements`** — тот же WebGL-баг, что
  чинили в релее: `LoadFriendsAsync` доходил до `IVhrProfile.ResolveNamesAsync` (резолв ников
  друзей) и зависал на продолжении после REST → список друзей грузился вечно. Список друзей теперь
  отображается. (`WebGLWebRtc` ещё содержит `ConfigureAwait(false)`, но WebRTC выключен —
  `PreferWebRtc=false`; починить при включении WebRTC.)
- Диагностический `[VHR WS] msg`-лог больше не печатает высокочастотные игровые data-кадры (`0x83`).
...
- **`WebGLVhrSocket`: делегаты JS-колбэков (`OnOpenStatic`/`OnMessageStatic`/`OnCloseStatic`) вынесены
  в статические поля.** Раньше они передавались в `VhrWs_Connect` как method-group → C# создавал
  ВРЕМЕННЫЕ делегаты, которые GC собирал после возврата вызова, и переданные в JS указатели
  становились висячими. Итог: браузерный WebSocket подключался (релей видел `/ws`, слал 101), но
  `onopen`/`onmessage` из JS НЕ доходили до C# → `VhrRelay.ConnectAsync` висел вечно на ожидании
  `0x81 SJoined`, лобби застревало на «Создание лобби…». Локализовано пошаговыми логами v1.7.4
  (последняя строка — `[VHR Relay] ConnectAsync →`, дальше тишина; `[VHR WS] onopen` не появлялся,
  хотя релей соединение держал открытым). В редакторе бага не было (нативный сокет без P/Invoke).
- Диагностические логи `[VHR Relay]`/`[VHR WS]` и 10с-таймаут на connect (из v1.7.4) оставлены.
